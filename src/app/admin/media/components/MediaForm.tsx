...
export default function MediaForm({ initial, onSave, onCancel, saving }: Props) {
  ...
  const set = <K extends keyof MediaFormData>(k: K, v: MediaFormData[K]) =>
    setForm((f) => ({ ...f, [k]: v }))

  const folder = FOLDER_MAP[form.type]

  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form) }} className="space-y-4">
      {/* Type first so folder hint reflects current selection */}
      <Field label="Type *">
        <select
          className={input}
          value={form.type}
      ...
      <FileUploadField
        ...
        onChange={(key) => set('filename', key ?? '')}
        hint={`uploads to ${folder}/`}
      />
      {/* Show R2 key for manual edits when file already exists */}
      {form.filename && (
        <Field label="R2 Key" hint="auto-set by upload — edit manually only if needed">
          <input
      ...
      <FileUploadField
        ...
        onChange={(key) => set('thumbnail', key)}
        hint="optional — auto-generated from video if not set"
      />
      ...
      <Field label="Created Date" hint="e.g. Spring 2024">
        <input
          ...
          onChange={(e) => set('created_date', e.target.value || null)}
        />
      </Field>
      ...
      <div className="flex gap-3 pt-2">
        <button type="submit" disabled={saving || !form.filename} className={primaryBtn}>
          {saving ? 'Saving…' : 'Save'}
        </button>
